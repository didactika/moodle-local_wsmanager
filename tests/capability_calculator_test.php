 ...
 * @covers     \local_servicemanager\automation\capability_calculator
 */
final class capability_calculator_test extends \advanced_testcase {
    ...
    protected function create_function_fixture(string $name, string $capabilities): void {
        ...
        $DB->insert_record('external_functions', [
            ...
            'methodname' => 'execute',
            'classpath' => '',
            'component' => 'local_servicemanager',
    ...
    public function test_get_function_capabilities(): void {
        $this->resetAfterTest();
        $this->create_function_fixture('local_servicemanager_fake_function', 'moodle/fake:cap1,moodle/fake:cap2');

        $calculator = new \local_servicemanager\automation\capability_calculator();
        $caps = $calculator->get_function_capabilities('local_servicemanager_fake_function');
    ...
    public function test_get_capabilities_for_functions(): void {
        $this->resetAfterTest();
        $this->create_function_fixture('local_servicemanager_fake_function', 'moodle/fake:cap1');

        $calculator = new \local_servicemanager\automation\capability_calculator();

        $functions = [
            ['name' => 'local_servicemanager_fake_function', 'critical' => true],
        ];
    ...
    public function test_get_capabilities_for_functions_with_string_names(): void {
        ...
        $calculator = new \local_servicemanager\automation\capability_calculator();
        $caps = $calculator->get_capabilities_for_functions(['local_servicemanager_fake_function']);
    ...
    public function test_deduplicate_capabilities(): void {
        $this->resetAfterTest();
        $this->create_function_fixture('local_servicemanager_fake_function_a', 'moodle/fake:shared,moodle/fake:onlya');
        $this->create_function_fixture('local_servicemanager_fake_function_b', 'moodle/fake:shared,moodle/fake:onlyb');

        $calculator = new \local_servicemanager\automation\capability_calculator();
        $caps = $calculator->get_capabilities_for_functions([
            'local_servicemanager_fake_function_a',
            'local_servicemanager_fake_function_b',
        ]);
